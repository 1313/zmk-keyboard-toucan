...
battery_peripheral_status_get_state(const zmk_event_t *eh) {
    ...
    if (ev == NULL) {
        ...
        return (struct battery_peripheral_status_state){.level = level};
    }
...
int zmk_widget_screen_init(struct zmk_widget_screen *widget, lv_obj_t *parent) {
    ...
    lv_canvas_set_buffer(top, widget->cbuf, CANVAS_W, CANVAS_H, CANVAS_COLOR_FORMAT);
